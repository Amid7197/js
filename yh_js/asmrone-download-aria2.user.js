...
(function () {
    ...
    function testConnection(host, port, secret) {
        ...
        GM_xmlhttpRequest({
                ...
                if (response.status >= 200 && response.status < 300) {
                    try {
                        ...
                        if (res.result && res.result.version) {
                            alert(`连接成功！Aria2 版本：${res.result.version}`);
                        } else if (res.error) {
                            alert(`连接失败：${res.error.message}`);
                        } else {
                            alert('收到响应但格式未知，请检查服务器');
                        }
                    } catch (e) {
                        alert('响应解析失败，可能不是 Aria2 服务');
                    }
                } else {
            ...
            onerror: function () {
                alert('网络错误，无法连接到服务器，请检查地址和端口');
            },
            ontimeout: function () {
                alert('连接超时，请检查服务器是否可达');
            }
        });
    }
    ...
    function openSettings() {
        ...
        overlay.innerHTML = `
            <div style="background: #fff; padding: 20px; border-radius: 8px;
                ...
                <label style="display:block; margin-bottom:2px; font-weight:500;">端口</label>
                <input id="aria2-port" type="text" value="${config.port}" style="width:100%; margin-bottom:12px; padding:8px; border:1px solid #ccc; border-radius:4px; box-sizing:border-box;">
                <label style="display:block; margin-bottom:2px; font-weight:500;">密钥（可选）</label>
                ...
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <div style="display:flex; gap:8px;">
                        <button id="aria2-cancel" style="padding:8px 16px; background:#6c757d; color:#fff; border:none; border-radius:4px; cursor:pointer;">取消</button>
                        <button id="aria2-save" style="padding:8px 16px; background:#28a745; color:#fff; border:none; border-radius:4px; cursor:pointer;">保存</button>
                    </div>
                </div>
    ...
    function addButtons() {
        ...
        downloadBtn.innerHTML = `<span class="q-btn__content text-center col items-center q-anchor--skip justify-center row">
            <i class="q-icon on-left notranslate material-icons">download</i>
        ...
        settingsBtn.innerHTML = `<span class="q-btn__content text-center col items-center q-anchor--skip justify-center row">
            <i class="q-icon on-left notranslate material-icons">settings</i>
            <span class="block">设置</span>
        </span>`;
    ...
    function addUri(uris, options) {
        ...
        const params = config.secret
            ? [`token:${config.secret}`, uris, options]
            : [uris, options];
        const body = {
            jsonrpc: '2.0',
            id: id,
